...
for i=1:ns
    for j=max(i-w,1):min(i+w,nt)
        oost=norm(s(i,:)-t(j,:));%edw ypologizetai h euclidean distance
       %% oost=MahalanobisDistance(s(i,:),t(j,:), myvariance); % edw ypologizetai h mahalanobis distance (not normalized)
       %%oost=mahal(s(i,:),t(:,:));
